...
class ComponentRegistry<T> {
    ...
    // Очередь для потокобезопасного доступа
    private let queue = DispatchQueue(label: "com.example.ComponentRegistry.queue")
        ...
        return queue.sync {
            if let version = version, let schemaUrl = schemaUrl {
                ...
                if componentByNameVersionSchema[name]![version]![schemaUrl] == nil {
                    componentByNameVersionSchema[name]![version]![schemaUrl] = buildComponent(InstrumentationScopeInfo(name: name,
                                                                                                                       version: version,
                                                                                                                       schemaUrl: schemaUrl))
                }
