...
public class DoubleExplicitBucketHistogramAggregator: StableAggregator {
  class Handle: AggregatorHandle {
    private let queue = DispatchQueue(label: "DoubleExplicitBucketHistogramAggregator.Handle.queue")
    ...
    // Внутренние свойства, доступ к которым осуществляется только внутри `queue.sync`
    ...
    override func doAggregateThenMaybeReset(startEpochNano: UInt64,
      ...
      return queue.sync {
        let pointData = HistogramPointData(
  ...
  public func toMetricData(resource: Resource, scope: InstrumentationScopeInfo, descriptor: MetricDescriptor, points: [PointData], temporality: AggregationTemporality) -> StableMetricData {
    StableMetricData.createHistogram(
      resource: resource,
      instrumentationScopeInfo: scope,
      name: descriptor.name,
      description: descriptor.description,
      unit: descriptor.instrument.unit,
      data: StableHistogramData(aggregationTemporality: temporality, points: points as! [HistogramPointData])
    )
  }
}
